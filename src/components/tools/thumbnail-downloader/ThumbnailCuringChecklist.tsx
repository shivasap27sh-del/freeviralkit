...
import { CheckSquare, Square, ShieldCheck, Sparkles, AlertTriangle } from 'lucide-react';
...
export function ThumbnailCuringChecklist() {
  ...
  return (
    <section aria-labelledby="audit-heading" className="glass-card rounded-3xl p-6 md:p-8 border border-blue-500/20 bg-slate-900/90 shadow-2xl space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
        <div>
          ...
          <h2 id="audit-heading" className="text-xl sm:text-2xl font-extrabold text-white mt-1">
            5-Point YouTube Thumbnail <span className="gradient-text">CTR Audit</span>
          </h2>
        </div>
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <span className="text-xs font-mono font-bold px-3 py-1 rounded-full bg-slate-950 border border-blue-500/30 text-cyan-300">
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border text-left transition-all cursor-pointer flex items-start gap-3 ${
                isDone
                  ? 'border-cyan-500 bg-blue-500/10 text-white shadow-md shadow-blue-500/10'
