    ...
    <main className="container mx-auto px-6 py-12 max-w-4xl relative z-10 min-h-screen">
      ...
      <section className="text-center mb-12">
        ...
        <p className="text-slate-600 text-lg max-w-2xl mx-auto">
          Generate 5 viral, high-retention concepts for YouTube Shorts and Reels. Get hook scripts, visual cues, and voiceover outlines.
        </p>
      </section>
      ...
      <section className="mt-12 space-y-8">
        <h2 className="font-display text-2xl font-bold">Why YouTube Shorts Need AI Concept Planning</h2>
        <div className="text-slate-600 leading-relaxed space-y-4">
          <p>
            With the rapid growth of YouTube Shorts, Instagram Reels, and TikTok, creators face intense competition for user attention. Viewers swipe away in a fraction of a second if they aren&apos;t instantly hooked.
          </p>
          <p>
            A viral YouTube Short relies on a formula: a strong visual hook in the first 3 seconds, snappy visual updates (B-roll or zoom-ins), and a concise voiceover. Our AI generator builds these components into every plan.
          </p>
        </div>
        <h3 className="font-display text-xl font-bold">Elements of a High-Retention YouTube Short</h3>
        <ul className="space-y-4 text-slate-600">
          <li className="flex items-start gap-2">
            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-pink-400 shrink-0" />
            <span>
              <strong>The Hook:</strong> A bold, intriguing opening statement (either on-screen text or spoken) that disrupts the swipe pattern.
            </span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-pink-400 shrink-0" />
            <span>
              <strong>Visual Pacing:</strong> Cuts, text highlights, zooms, or B-roll changes happening every 1.5 to 3 seconds to keep the eye active.
            </span>
          </li>
          <li className="flex items-start gap-2">
            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-pink-400 shrink-0" />
            <span>
              <strong>Audio Syncing:</strong> Upbeat background music paired with clean voiceovers. The script should be conversational and energetic.
            </span>
          </li>
        </ul>

        <div className="glass-card rounded-2xl p-6 text-center">
          <p className="text-slate-600 mb-4">Need SEO titles, descriptions, hashtags, and tags for your Shorts? We have you covered.</p>
          <Link href="/" className="inline-flex items-center gap-2 btn-primary rounded-xl px-6 py-3 font-semibold">
            Try Full SEO Optimizer <Sparkles className="w-4 h-4" />
          </Link>
        </div>
      </section>
    </main>
