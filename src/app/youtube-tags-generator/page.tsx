...
export default function TagsGeneratorPage() {
  ...
  const handleGenerate = async (val?: string, isRegenerate = false) => {
    ...
    setTags([]);
    setError(null);
    const result = await generateTagsOnly(inputVal, exclude);
  ...
  const examples = ['Unboxing Tech', 'Python Guide', 'Beginner Yoga', 'Minecraft Build'];
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8 mb-8">
        <div className="relative mb-4">
          <input type="text" value={topic} onChange={e => setTopic(e.target.value)}
            placeholder="Enter your video topic or title (e.g. how to code in javascript, diy projects...)"
        ...
        {tags.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-2xl p-6 md:p-8 mb-8">
            ...
            <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-2xl p-5">
              <h4 className="font-display font-semibold text-cyan-400 mb-1 flex items-center gap-1.5">
