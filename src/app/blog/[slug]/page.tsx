              ...
              <li key={j} className="flex items-start gap-2 text-slate-700 dark:text-slate-300 leading-relaxed">
                ...
                <span
                  dangerouslySetInnerHTML={{
                    __html: item
                      .replace(/^(?:-\s*|\*\s+|❌\s*|✅\s*)/, '')
                      .replace(/!\[(.*?)\]\((.*?)\)/g, '<img src="$2" alt="$1" width="800" height="400" loading="lazy" decoding="async" onerror="this.style.display=\'none\'" class="rounded-xl my-6 max-w-2xl mx-auto max-h-[360px] object-cover border border-slate-200 dark:border-slate-800 block" />')
